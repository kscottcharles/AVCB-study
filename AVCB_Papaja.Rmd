...
AVCB_H1$Years <- factor(AVCB_H1$Years)
...
p1_noAge <- ggplot(
  data = AVCB_H1,
  mapping = aes(x = Parent_AVscore, y = Child_AVscore)) +
  geom_smooth(method = "lm") +
  geom_jitter() +
    labs(
      title = "Relation Between Parents' and Children's Authoritarian Values",
...
AVCB_H2$Years <- factor(AVCB_H2$Years)
  
p2a <- AVCB_H2 %>% filter(!is.na(Child_Class_Self_choice)) %>% 
ggplot(aes(x = Child_Class_Self_choice, y = COBRAS_avg, fill = Child_Class_Self_choice)) +
  facet_grid(cols = vars(Years),
               labeller = labeller(Years = c("6" = "6 years", "7" = "7 years", "8" = "8 years", "9" = "9 years", "10" = "10 years"))) +
  geom_boxplot() +
  geom_jitter() +                                         
  ...
  coord_flip() +   
  scale_fill_manual(values = c("CB" = "#6baed6", "CC" = "#2171b5"))
...
ggplot(aes(x = factor(Years), y = COBRAS_avg, fill = Child_Class_Self_choice)) +
  facet_grid(cols = vars(Child_Class_Self_choice),
               labeller = labeller(Child_Class_Self_choice = c("CB" = "Child Picked Colorblind Class", "CC" = "Child Picked Color-Conscious Class"))) +
  geom_boxplot() +
  geom_jitter() +                                         
    labs(
      title = "Relation Between Children's Classroom Choice and Parents' Colorblind Score \nBy Child's Age",
...
ggplot(aes(x = Child_Class_Self_choice, y = Child_AVscore, fill = Child_Class_Self_choice)) +
...
In Table \@ref(r dem table), we can see the summary of parents' education levels.
...
######Rel between parent measures
  ...
  ##Rel between diversity ideology measures - NEED TO MAKE PLOT FOR THESE
...
#Linear regression model:
  ...
  ##Rel between parent diversity ideology measures and AV measure - P3A

#Parent AV + Teacher and Self classroom choice (binary) - NEED TO MAKE PLOT FOR THIS
...
#check cor first
AVCB_H3_no_missing <- na.omit(AVCB_H3[, c("Parent_AVscore", "COBRAS_avg")])
cor.test(AVCB_H3_no_missing$Parent_AVscore, AVCB_H3_no_missing$COBRAS_avg)

#Linear regression model

pAV_pCOBRAS_lm <- lm(Parent_AVscore ~ COBRAS_avg, data = AVCB_H3)
...
#Parent-perception (binary) and AV - NEED TO MAKE PLOT FOR THIS (ONLY MADE FOR SELF CLASSROOM CHOICE)
  #Logistic regression model:
  #choice on the Teacher measure as the outcome and authoritarian values as the predictor

#Self classroom choice (binary) and AV - P3B
  #Logistic regression model:
  #choice on the Teacher measure as the outcome and authoritarian values as the predictor
...
#Parent AV and children's AV - p1_noAge (only uses SELF AV for both; need to create another for parent-perception)
...
# Check if "Parent_AVscore" is present in the coefficients
...
The linear model shows that for every additional point of parents' authoritarian values score, parents' COBRAS score increases by `r coefficients(pAV_pCOBRAS_lm)[["COBRAS_avg"]]` points, and this effect is significant `pAV_pCOBRAS_lm$p.value`.
